...
const getCredentials = request => {
  ...
  if(!request.headers.authorization){
    ...
    const toBeDecoded = request.headers.authorization.split(' ');
    if(toBeDecoded[0] !== 'Basic'){
      return null;
    }
    else{
    const buff = Buffer.from(toBeDecoded[1], 'base64');
    const text = buff.toString('ascii');
    const result = text.split(':');
    const resultArr = [result[0], result[1]];
    return resultArr;
    }
  }
