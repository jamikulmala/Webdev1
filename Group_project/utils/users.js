...
const getUser = (email, password) => {
  const user = data.users.find(userr => userr.email === email && userr.password === password);
  return user && {...user };
};
...
const getUserById = userId => {
  for(const user in data.users){
    if(data.users[user]._id === userId){
      const koira = {...data.users[user]};
      return {...koira};
    }
  }
  return undefined;
...
const updateUserRole = (userId, role) => {
  const daa = data.roles.find(roles => {
    return roles === role;
  });
  if(daa !== undefined){
