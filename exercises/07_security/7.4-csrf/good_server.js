...
http.createServer(function(request, response) {
  ...
  // The login page directs here, where user's credentials are checked
    ...
    // If not, send the user to the money transfer form again
      ...
      // As the hidden field is inside a template literal below, we could add expression with (from https://developer.mWozilla.org/en-US/docs/Web/JavaScript/Reference/Template_literals):       `string text ${expression} string text`
...
const checkCSRFtoken = (token) => {
  var index = csrfTokens.findIndex(e => e === token);
  if(index !== -1){
